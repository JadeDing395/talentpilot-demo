...
import { extractJobListings } from "@/lib/jd-scraper";
...
export async function POST(req: NextRequest) {
  let body: { url?: string; position?: string; keywords?: string[]; aiConfig?: AiClientConfig };
  try {
    body = await req.json();
  } catch {
  ...
  try {
    ...
    const result = await extractJobListings(userId, {
      url: body.url,
